{ config, pkgs, ... }: {
	programs.fzf.enableZshIntegration = true;
	programs.eza.enableZshIntegration = true;
	programs.zoxide.enableZshIntegration = true;
	programs.oh-my-posh.enableZshIntegration = true;

	programs.zsh = {
		enable = true;

		history = {
			share = true;
			size = 10000;
			save = 10000;
			path = "$HOME/.zsh_history";

			ignoreSpace = true;
			ignoreDups = true;
			ignoreAllDups = true;
		};

		antidote.enable = true;
		antidote.plugins = [
			"zsh-users/zsh-syntax-highlighting"
			"zsh-users/zsh-completions"
			"zsh-users/zsh-autosuggestions"
			"aloxaf/fzf-tab"
		];

		shellAliases = {
			nix-rebuild-default = "sudo nixos-rebuild switch --flake \"path://${config.home.homeDirectory}/.sources/nix/#default\"";
			ls = "eza --color=always --long --git --no-filesize --icons=always --no-time --no-user --no-permissions";
			tree = "eza --tree --level=2";
			cd = "z";
			cat = "bat -p";
		};

		initExtraFirst = ''
# Tmux is the first thing that should be evaluated
if [[ ! -n $TMUX ]]; then
	(tmux ls > /dev/null 2>&1 && tmux attach) || tmux new-session
fi

# After tmux comes the shell
eval "$(oh-my-posh init zsh --config ~/.config/oh-my-posh/config.toml)"

# After the shell we set the keybinds
bindkey -v

# FZFGIT package, actually sourcing it from a github
FZFGIT_DIR="${config.home.homeDirectory}/.fzf-git/"
if [ -f "''${FZFGIT_DIR}/fzf-git.sh" ]; then
	source "''${FZFGIT_DIR}/fzf-git.sh"
fi

## Autogenerated by Nix ##
		'';

		initExtra = ''
## Autogenerated by Nix END ##

show_file_or_dir_preview="if [ -d {} ]; then eza --tree --color=always {} | head -200; else bat -n --color=always --line-range :500 {}; fi"

export FZF_DEFAULT_COMMAND="fd --hidden --strip-cwd-prefix --exclude .git"
export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND"
export FZF_ALT_C_COMMAND="fd --type=d --hidden --strip-cwd-prefix --exclude .git"

export FZF_CTRL_T_OPTS="--preview '$show_file_or_dir_preview'"
export FZF_ALT_C_OPTS="--preview 'eza --tree --color=always {} | head -200'"

_fzf_comprun() {
	local command=$1
	shift

	case "$command" in
		cd)						fzf --preview "eza --tree --color=always {} | head -200" "$@" ;;
		export|unset)	fzf --preview "eval \'echo \\$\'" ;;
		ssh)					fzf --preview "dig {}" ;;
		*)						fzf --preview "$show_file_or_dir_preview" "$@" ;;
	esac
}
		'';
	};

	home.file.".fzf-git/".source = builtins.fetchGit {
		url = "https://github.com/junegunn/fzf-git.sh.git";
		rev = "bd8ac4ba4c9d7d12b34f7fa2b0d334f50cdb5254";
	};
}
